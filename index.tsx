...
function CustomCursor() {
  useEffect(() => {
    ...
    const raf = () => { rx+=(mx-rx)*.18; ry+=(my-ry)*.18;
      ring.style.transform = `translate(${rx}px,${ry}px) translate(-50%,-50%)`; requestAnimationFrame(raf); };
...
function Contact({ prefill, onSent }: { prefill: string; onSent: ()=>void }) {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    setSending(true);
    setTimeout(() => { setSending(false); onSent(); setMsg(""); }, 1400);
  };
  return (
      ...
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12">
        ...
        <form id="contact-form" onSubmit={submit} className="reveal bg-white border border-[var(--border)] rounded-3xl p-8 md:p-10 shadow-[var(--shadow-card)] space-y-5">
          <div>
            ...
            <input required className="w-full bg-[color:var(--secondary)] border border-transparent focus:border-[color:var(--primary)] focus:bg-white outline-none rounded-xl px-4 py-3.5 transition-all" />
          </div>
          <div className="grid sm:grid-cols-2 gap-5">
            <div>
              <label className="text-sm font-semibold block mb-2">Email</label>
              <input required type="email" className="w-full bg-[color:var(--secondary)] border border-transparent focus:border-[color:var(--primary)] focus:bg-white outline-none rounded-xl px-4 py-3.5 transition-all" />
            </div>
            <div>
              <label className="text-sm font-semibold block mb-2">Phone (optional)</label>
              <input className="w-full bg-[color:var(--secondary)] border border-transparent focus:border-[color:var(--primary)] focus:bg-white outline-none rounded-xl px-4 py-3.5 transition-all" />
            </div>
          </div>
          <div>
          ...
          <button disabled={sending} className="btn-primary w-full justify-center disabled:opacity-70">
            {sending ? (<><span className="w-4 h-4 rounded-full border-2 border-white/40 border-t-white animate-spin" />Sending…</>) : (<>Submit <I.arrow className="w-4 h-4" /></>)}
          </button>
        </form>
      </div>
    </section>
      ...
      <div className="max-w-7xl mx-auto px-6 py-14 grid md:grid-cols-4 gap-10">
        <div className="md:col-span-2">
          <div className="flex items-center gap-2.5 font-display font-black text-xl text-white">
            <span className="w-9 h-9 rounded-xl grid place-items-center" style={{ background: "var(--gradient-primary)" }}>
              <I.cpu className="w-5 h-5" />
            </span>
